...
def loadKNNDataAndTrainKNN():
    ...
    for dirr in os.listdir(path):
        for file in os.listdir(path + dirr + '/'):
            ...
            npaFlattenedImages.append(np.array(gray).reshape((336)))
        
    kNearest.setDefaultK(1)                                                             # set default K to 1

    kNearest.train(np.array(npaFlattenedImages).astype(np.float32), 0, np.array(npaClassifications).astype(np.float32))          # train KNN object
